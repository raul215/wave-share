...
// static byte key[CryptoPP::AES::DEFAULT_KEYLENGTH], iv[CryptoPP::AES::BLOCKSIZE];
static byte iv[CryptoPP::AES::BLOCKSIZE];
static const byte key[] = {0x3c, 0x5a, 0xc8, 0x12, 0x61, 0xc1, 0x1a, 0x19, 0x0b, 0xc3, 0xe9, 0x20, 0x4a, 0xb9, 0x98, 0x97};
            ...
            if (nBytesRecorded != 0)
            {
                        ...
                        if (knownLength)
                        {
                            ...
                            if (rsData->Decode(encodedData.data() + encodedOffset, rxData.data()) == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    stfDecryptor.Put(reinterpret_cast<const unsigned char *>(s.c_str()), s.size());
                ...
                if (receivingData == false)
                {
                    ...
                    for (int i = 0; i < nBitsInMarker; ++i)
                    {
                        ...
                        if (i % 2 == 0)
                        {
                            if (sampleSpectrum[bin] <= 3.0f * sampleSpectrum[bin + d0])
                                isReceiving = false;
                        }
                        else
                        {
...
int main(int argc, char **argv)
{
    ...
    case 3:
    {
        printf("Using 'Ultrasonic 310' Tx Protocol\n");
        setParameters(1, 310, 9, 3, 0, 50);
    }
    break;
    case 4:
    {
        printf("Using 'Ultrasonic 320' Tx Protocol\n");
        setParameters(1, 320, 9, 3, 0, 50);
    }
    break;
    case 5:
    {
        printf("Using 'Ultrasonic 330' Tx Protocol\n");
        setParameters(1, 330, 9, 3, 0, 50);
    }
    break;
    case 6:
    {
        printf("Using 'Ultrasonic 340' Tx Protocol\n");
        setParameters(1, 340, 9, 3, 0, 50);
    }
    break;
    case 7:
    {
        printf("Using 'Ultrasonic 350' Tx Protocol\n");
        setParameters(1, 350, 9, 3, 0, 50);
    }
    break;
    case 8:
    {
        printf("Using 'Ultrasonic 360' Tx Protocol\n");
        setParameters(1, 360, 9, 3, 0, 50);
    }
    break;
    case 9:
    {
        printf("Using 'Ultrasonic 370' Tx Protocol\n");
        setParameters(1, 370, 9, 3, 0, 50);
    }
    break;
    case 10:
    {
        printf("Using 'Ultrasonic 380' Tx Protocol\n");
        setParameters(1, 380, 9, 3, 0, 50);
    }
    break;
    default:
    {
    ...
    std::thread inputThread([]() {
        ...
        while (true)
        {
            ...
            setText(ciphertext.size(), ciphertext.data());
            inputOld = ciphertext;
        }
    });
